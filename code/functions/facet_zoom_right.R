...
# facet_zoom_right.R
#
# The facet_zoom() function from the ggforce package positions the zoomed panel on the left side of the main
# panel. The code for the function (https://github.com/thomasp85/ggforce/blob/main/R/facet_zoom.R) was modified
# based on suggestions from a Stack Overflow answer
# (https://stackoverflow.com/questions/52665619/how-to-change-the-position-of-the-zoomed-area-from-facet-zoom)
# to position the zoomed panel on the right side. The modified lines of code are quoted below with ##.
#
#################################################################################################################
...
facet_zoom_right <- function(x, y, xy, zoom.data, xlim = NULL, ylim = NULL,
...
##                                                from = y_scales[[1]]$dimension(expansion(y_scales[[1]]))
                           zoom_prop <- scales::rescale(y_scales[[2]]$dimension(expansion(y_scales[[2]])),
                                                        from = y_scales[[1]]$dimension(expansion(y_scales[[1]]))
                           )
...
##                                                from = x_scales[[1]]$dimension(expansion(x_scales[[1]]))
                           zoom_prop <- scales::rescale(x_scales[[2]]$dimension(expansion(x_scales[[2]])),
                                                        from = x_scales[[1]]$dimension(expansion(x_scales[[1]]))
                           )
                           indicator <- polygonGrob(
                             c(zoom_prop, 1, 0),
